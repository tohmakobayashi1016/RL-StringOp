...
from Mesh_Environment_simplified_hybrid_mse_A_to_B_final import MeshEnvironment
import time, torch
...
sweep_config = {
    ...
    'metric': {
        'name': 'mean_reward',  # Metric to optimize
        'goal': 'maximize'
    },
    'parameters': {
        'learning_rate': {
...
def train_dqn_with_wandb():
    with wandb.init() as run:
        ...
        env = MeshEnvironment(initial_mesh, terminal_mesh_json_path, max_steps = 7)
        ...
        model = DQN(
            ...
            exploration_final_eps=0.05, # Default = 0.05 Make it smaller
            verbose=1
        )

        # Define the number of timesteps for training
        total_timesteps = 1000  # Adjust this based on your needs
        ...
        wandb.log({'final_loss': model.mean_loss, 'total_timesteps': total_timesteps})
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep_config, project="DQN-A-B-Steps")
