...
Topological features: Histogram of singularity degrees 

Reward function: Cross-entropy (CE), ...
...
class MeshEnvironment(gym.Env):
    ...
    def step(self, action):
        ...
        try:
            ...
            print(f"Final observation: {self.get_state()[0]}")
    ...
    def calculate_reward(self, done, truncated):
        ...
        # Convert histograms to tensors for cross-entropy loss calculation
        current_histogram_tensor = torch.tensor([
        ...
        terminal_histogram_tensor = torch.tensor([
            len(self.terminal_histogram['degree_2_vertices']),
            len(self.terminal_histogram['degree_3_vertices']),
            len(self.terminal_histogram['degree_4_vertices']),
            len(self.terminal_histogram['degree_5_vertices']),
            len(self.terminal_histogram['degree_6_plus_vertices'])
        ], dtype=torch.float32)

        # Compute cross-entropy loss
        cross_entropy_loss = F.cross_entropy(current_histogram_tensor.unsqueeze(0), terminal_histogram_tensor.argmax().unsqueeze(0))
        print(f"This is the cross-entropy loss: {cross_entropy_loss.item()}")
        ...
        if done or truncated:
            ...
            if identical_to_initial:
                ...
                histogram_change = -20.0
            else:
                histogram_change = -time_step_penalty*self.max_steps + cross_entropy_loss.item()*self.max_steps
                print(f"Final design step is changed, applying reward: {histogram_change}")

        # Total reward: negative cross-entropy + time-step penalty + distance reward
        reward = -cross_entropy_loss.item() + time_step_penalty + histogram_change
        print(f"This is the total reward: {reward}")

        # Additional reward for reaching terminal state
        if done:
            reward += 205
            print(f"Big reward has been added.")
        return reward
