...
Reward function: Kullback-Leibler divergence (KL), ...
...
class MeshEnvironment(gym.Env):
    ...
    def calculate_reward(self, done, truncated):
        ...
        # Only consider degrees (2 or 3) and higher
        ...
        current_histogram_tensor = torch.tensor([len(current_histogram[key]) for key in degrees_to_consider], dtype=torch.float32)
        terminal_histogram_tensor = torch.tensor([len(self.terminal_histogram[key]) for key in degrees_to_consider], dtype=torch.float32)

        # Normalize both histograms to get probabilities
        epsilon = 1e-10
        current_distribution = (current_histogram_tensor + epsilon) / (current_histogram_tensor.sum() + epsilon)
        terminal_distribution = (terminal_histogram_tensor + epsilon) / (terminal_histogram_tensor.sum() + epsilon)

        # Compute KL-Divergence between the current and terminal histograms
        ...
        if done or truncated:
            ...
            if identical_to_initial:
                ...
                histogram_change = -time_step_penalty*self.max_steps + kl_divergence.item()*self.max_steps
        ...
        reward = -kl_divergence.item() + time_step_penalty + histogram_change
        print(f"This is the total reward: {reward}")

        # Reward for reaching terminal state (if done)
        if done:       
